...
function banterFor(a, b, ta, tb) {
  const nameA = leaderDisplayName(a);
  const nameB = leaderDisplayName(b);

  if (nameA === nameB) {
    return `${nameA} has both teams here. Guaranteed points, guaranteed smugness.`;
  }
  ...
  return lines[pair] || `${nameA} versus ${nameB}. ${ta} and ${tb} have been dragged into family politics.`;
}
